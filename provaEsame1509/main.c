...
void DB_process_routine(char * database_file_name, int shm_id, int shmOUT_id, int sem_id, int numb_process ){
    ...
    //troviamo il numero di righe del file database
    ...
    while(fgets(str, MAX_LEN_CONTENT, file_id)){
        sscanf(str, "%[^:]:%d", database[j].nome, &database[j].valore);
    ...
    while(1){
        ...
        {   
            WAIT(sem_id, OUT, 1);
                shmOUT->process = process_id;
                strcpy(shmOUT->str, end);
                shmOUT->valore = valore;
            SIGNAL(sem_id, DB1, 1);
            printf("DB STO FINENDO\n");
            fflush(stdout);
            return;
        }
